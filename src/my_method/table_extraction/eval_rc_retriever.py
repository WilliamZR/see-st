...
import torch
from rc_data_generator import RC_Generator
...
if __name__ == '__main__':
    ...
    print(args)
    assert args.eval_split in ['train', 'dev', 'test']
    tokenizer = AutoTokenizer.from_pretrained(args.tapas_path)
    gold_evidence_by_id = get_gold_table_evidence(args.eval_split)
    ...
    valid_data = RC_Generator(args.input_path, tokenizer, args.eval_split, args)
    ...
    print(args.eval_split)
    print('Table Retrieval:')
    print(recall)

    with jsonlines.open(args.input_path + '/' + args.eval_split + '.rc.p5.t{0}.times.jsonl'.format(args.max_tabs), 'w') as writer:
        for id in predicted_tables_by_id:
            writer.write({'id':id,                                  'predicted_tables':predicted_tables_by_id[id]})
