...
class BiModalClsConcat(BasicModule):
    def __init__(self, args):
        super(BiModalClsConcat, self).__init__()
        ...
        self.relu = nn.ReLU()
        self.linear2 = nn.Linear(128*2, len(args.id2label))
    ...
    def forward(self, batch, args, test_mode):
        ...
        hg2 = self.relu_2(self.linear1_2(hg))
        # hg = self.linear2_2(hg)
        # hg2 = hg

        hg = self.linear2(torch.cat([hg1, hg2], dim=-1))
